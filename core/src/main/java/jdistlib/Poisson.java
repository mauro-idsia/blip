...
public class Poisson extends GenericDistribution {
    ...
    private static double density(double x, double lambda, boolean give_log) {
        ...
        if (x < 0 || MathFunctions.isInfinite(x)) {
            return (give_log ? Double.NEGATIVE_INFINITY : 0.);
        }
    ...
    private static double do_search(double y, double[] z, double p, double lambda, double incr) {
        if (z[0] >= p) {
            ...
            for (;;) {
                if (y == 0
                        || (z[0] = cumulative(y - incr, lambda, /* l._t.*/true, /* log_p*/
                        false))
                                < p) {
                    return y;
                }
            ...
            for (;;) {
                y = y + incr;
                if ((z[0] = cumulative(y, lambda, /* l._t.*/true, /* log_p*/
                        false))
                        >= p) {
                    return y;
                }
    ...
    private static double quantile(double p, double lambda, boolean lower_tail, boolean log_p) {
        ...
        if (!lower_tail || log_p) {
            ...
            p = (log_p
                    ...
                    : (lower_tail ? (p) : (0.5 - (p) + 0.5))); /* need check again (cancellation!): */
            if (p == 0.) {
                return 0;
            }
            if (p == 1.) {
    ...
    private static double random(double mu, RandomEngine random, RandomState state) {
        ...
        if (!(big_mu && mu == state.muprev)) { /* maybe compute new persistent par.s */
            ...
            if (big_mu) {
                ...
                for (;;) {
                    ...
                    if (state.l != 0) {
                        for (k = (u <= 0.458) ? 1 : min(state.l, state.m); k
                                <= state.l; k++) {
                            if (u <= state.pp[k]) {
                                return (double) k;
                            }
        ...
        for (;;) {
            ...
            fx = -0.5 * x;
            fy = state.omega
                    * (((state.c3 * x + state.c2) * x + state.c1) * x + state.c0);
            if (kflag) {
            ...
            } else /* Step Q. Quotient acceptance (rare case) */ if (fy - u * fy
                    <= py * exp(px - fx)) {
                break;
            }
        }
