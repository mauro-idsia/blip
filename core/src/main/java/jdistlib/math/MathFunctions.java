...
public class MathFunctions {
    ...
    public static final boolean isFinite(double x) {
        return !Double.isNaN(x) && (x != Double.POSITIVE_INFINITY)
    ...
    public static final double lgammafn_sign(double x, int[] sgn) {
        ...
        ans = M_LN_SQRT_PId2 + (x - 0.5) * log(y) - x - log(sinpiy)
                - lgammacor(y);
    ...
    public static final double stirlerr(double n) {
        ...
        final double sferr_halves[] = {
            ...
            0.1534264097200273452913848, /* 0.5 */ 0.0810614667953272582196702, /* 1.0 */
            0.0548141210519176538961390, /* 1.5 */
            0.0413406959554092940938221, /* 2.0 */ 0.03316287351993628748511048, /* 2.5 */
            0.02767792568499833914878929, /* 3.0 */
            0.02374616365629749597132920, /* 3.5 */ 0.02079067210376509311152277, /* 4.0 */
            0.01848845053267318523077934, /* 4.5 */
            0.01664469118982119216319487, /* 5.0 */ 0.01513497322191737887351255, /* 5.5 */
            0.01387612882307074799874573, /* 6.0 */
            0.01281046524292022692424986, /* 6.5 */ 0.01189670994589177009505572, /* 7.0 */
    ...
    public static final double bd0(double x, double np) {
        ...
        if (isInfinite(x) || isInfinite(np) || np == 0.0) {
            return Double.NaN;
        }
    ...
    public static final double lbeta(double a, double b) {
        ...
        if (p >= 10) {
            ...
            return lgammafn(p) + corr + p - p * log(p + q)
                    + (q - 0.5) * log1p(-p / (p + q));
        } else /* p and q are small: p <= q < 10. */ if (p < 1e-306) {
    ...
    public static final double[] bratio(double a, double b, double x, double y, boolean log_p) {
        ...
        w = w1 = (log_p
                ? Double.NEGATIVE_INFINITY
                : 0.);
        if (Double.isNaN(x) || Double.isNaN(y) || Double.isNaN(a)
                || Double.isNaN(b)) {
            return new double[] { w, w1, 9};
        }

        if (a < 0.0 || b < 0.0) {
            return new double[] { w, w1, 1};
        }
        if (a == 0.0 && b == 0.0) {
            return new double[] { w, w1, 2};
        }
        if (x < 0.0 || x > 1.0) {
            return new double[] { w, w1, 3};
        }
        if (y < 0.0 || y > 1.0) {
            return new double[] { w, w1, 4};
        }

        z = x + y - 0.5 - 0.5;

        if (abs(z) > eps * 3.0) {
            return new double[] { w, w1, 5};
        }

        if (x == 0.) {
            if (a == 0.0) {
                return new double[] { w, w1, 6};
            }
        ...
        if (min(a, b) <= 1.) {
            ...
            if (b0 < min(eps, eps * a0)) { /* L80: */
                ...
                w1 = log_p
                        ? ((w) > -M_LN2 ? log(-expm1(w)) : log1p(-exp(w)))
                        : 0.5 - w + 0.5;
                // goto L_end;
                if (do_swap) { /* swap */
                    double t = w;
        ...
        case 110:
            ...
            w = log_p
                    ? ((w1) > -M_LN2 ? log(-expm1(w1)) : log1p(-exp(w1)))
                    : 0.5 - w1 + 0.5;
            // goto L_end_after_log;
            break;
    ...
    public static final double bpser(double a, double b, double x, double eps, boolean log_p) {
        ...
        if (ans == (log_p ? Double.NEGATIVE_INFINITY : 0.)
                || (!log_p && a <= eps * 0.1)) {
            return ans;
        }
        ...
        if (abs(w) > tol) { // the series did not converge (in time)
            ...
            if ((log_p
                    && !(a * sum > -1. && abs(log1p(a * sum)) < eps * abs(ans)))
                            || (!log_p && abs(a * sum + 1) != 1.)) {
                System.err.println(
                        String.format(
    ...
    public static final double bup(double a, double b, double x, double y, int n, double eps, boolean give_log) {
        ...
        if (n == 1 || (give_log && ret_val == Double.NEGATIVE_INFINITY)
                || (!give_log && ret_val == 0.)) {
            return ret_val;
        }
        nm1 = n - 1;
    ...
    public static final double erf__(double x) {
        ...
        final double b[] = {
            .00301048631703895, .0538971687740286,
            .375795757275549};
        final double p[] = {
        ...
        final double s[] = {
            94.153775055546, 187.11481179959, 99.0191814623914,
            18.0124575948747};
    ...
    public static final double gam1(double a) {
        final double p[] = {
            .577215664901533, -.409078193005776,
    ...
    public static final double gamln(double a) {
        ...
        if (a <= 0.8) {
            ...
            double t = 1. / (a * a);
            double w = (((((c5 * t + c4) * t + c3) * t + c2) * t + c1) * t + c0)
                    / a;
    ...
    public static final double lchoose(double n, double k) {
        ...
        if (abs(k - k0) > 1e-7) {
            ...
            System.err.println(
                    String.format("'tw' (%.2f) must be integer, rounded to %.0f",
                    k0, k));
        }
        if (k < 2) {
            if (k < 0) {
     ...
     * Numerical Analysis Dundee, 1975, Base. A. Watson (ed.),
    ...
    public static final double gamma_cody(double x) {
        ...
        /* *******************************************************************
         ...
         VAX Base-Format	(D.P.)	      2		1023	    171.489
         ...
         VAX Base-Format	(D.P.)	 8.98D+307    1.11D-16	  1.12D-308
