...
public class SoftMissingBDeu
    ...
    public double computeScore(int n, int[] set_p) {
        ...
        TIntArrayList[] comp_values = computeParentSetValues(set_p, this.completion);
        ...
        skore = skore + (Gamma.lgamma(a_ij) * p_arity - Gamma.lgamma(a_ijk) * p_arity * arity);
        for (int p_v = 0; p_v < p_values.length; p_v++) {
            ...
            for (int v = 0; v < arity; v++) {
                double weight_n = ArrayUtils.intersectN(this.dat.row_values[n][v], p_values[p_v]);
                TIntArrayList inter = ArrayUtils.intersect(this.completion[n][v], comp_values[p_v]);
