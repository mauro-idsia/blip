...
//                }
    ...
    private boolean go(Result res, TIntHashSet todo) {
        ...
        for (int elim : h.set) {
            addHandler(new SIntSet(ArrayUtils.reduceAndIncreaseArray(h.set, res.v, elim)));
        }
        ...
        if (elapsed > cnt * 10) {
            RandomStuff.pf ("Thread %d, remaining to look: %d, score: %d - %.2f \n", thread, todo.size(), new_sk, elapsed);
            cnt += 1;
        }
        return false;
    }
    ...
    private void initClique() {
        ...
        for (int i1 = 0; i1 < initCl.length; i1++) {
            ...
            for (int i2 = i1 + 1; i2 < initCl.length; i2++) {
                ...
                if (ArrayUtils.find(v1, und.neigh[v2])) {
                    new_und.mark(v1, v2);
                    new_sk += 1;
                    // pf("Marking %d %d \n", v1, v2);
                }
        ...
        // Add new handlers
        for (int v : initCl) {
            addHandler(new SIntSet(ArrayUtils.reduceArray(initCl,v)));
        }
    }
    ...
    private int getSk(int[] neigh, int[] set, int v) {
        int sk = 0;
        for (int n: neigh) {
            if (!ArrayUtils.find(n, new_und.neigh[v]) && ArrayUtils.find (n, set))
                sk += 1;
        }
        return sk;
    }
