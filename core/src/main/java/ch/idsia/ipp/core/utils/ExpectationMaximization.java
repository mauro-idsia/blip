...
public class ExpectationMaximization extends Analyzer {
    ...
    public ExpectationMaximization(DataFileReader dat) {
        super(dat);
    }
    ...
    public short[] performEM(DataFileReader dat_rd, int missingVar, int[] set_p) {

        this.ar = dat_rd.l_n_arity[missingVar];

        new_values = new short[dat_rd.sample[missingVar].length];
        ArrayUtils.cloneArray(dat_rd.sample[missingVar], new_values);

        this.missing_row = dat_rd.row_values[missingVar][ar];
        this.p_values = computeParentSetValues(set_p);

        this.set_p = set_p;
        this.arities = dat_rd.l_n_arity;
        ...
        for (int i = 0; i < missing_row.length; i++) {
            ...
            for (int p_v = 0; p_v < p_values.length && !found; p_v++) {
                if (ArrayUtils.find(missing_row[i], p_values[p_v])) {
                    missing_pv[i] = p_v;
                    found = true;
                }
            }
